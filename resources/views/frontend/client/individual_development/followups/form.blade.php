...
<div class="container-fluid px-2 px-lg-3 idpf">
    <div class="head">
        ...
        <div class="actions">
            @if($readOnly)
                <span class="readonly"><i class="bi bi-eye me-1"></i>อ่านอย่างเดียว</span>
            @endif
    ...
    @if($previousFollowup?->next_action)
        <div class="prevbox">
            ...
            @if($previousFollowup->next_followup_date)
                <div class="small text-muted mt-2">กำหนดติดตามเดิม: {{ $thaiDate($previousFollowup->next_followup_date) }}</div>
            @endif
        </div>
    @endif

    @if(!empty($goals) && $goals->isNotEmpty())
        <div class="card">
            <div class="card-head"><div><div class="card-title">เป้าหมายที่ใช้ประกอบการติดตาม</div><div class="small text-muted mt-1">ระบบแสดงคะแนนล่าสุดเทียบ Target เพื่อช่วยสังเกตความก้าวหน้า แต่ผู้ปฏิบัติงานเป็นผู้ยืนยันผล</div></div></div>
            <div class="card-body"><div class="goal-summary">
                @foreach($goals as $goal)
                    @php
                        $gp = $goalProgress[$goal->id] ?? [];
                    @endphp
                    <div class="goal-mini"><div class="goal-mini-title">{{ $goal->domain?->name ?? '-' }} • {{ $goal->indicator?->name ?? $goal->title }}</div><div class="goal-mini-score">Baseline {{ $gp['baseline'] ?? $goal->baseline_level ?? '-' }} → ล่าสุด {{ $gp['current'] ?? '-' }} → Target {{ $gp['target'] ?? $goal->target_level ?? '-' }} • {{ ['not_started'=>'ยังไม่เริ่ม','in_progress'=>'กำลังดำเนินการ','partial'=>'มีความก้าวหน้า','achieved'=>'บรรลุแล้ว','cancelled'=>'ยกเลิก'][$goal->status] ?? $goal->status }}</div></div>
                @endforeach
            </div></div>
        </div>
    @endif
    ...
    <form data-idp-th-validation="1" method="POST" action="{{ $isEdit ? route('individual-development.followups.update',[$client->id,$followup->id]) : route('individual-development.followups.store',$client->id) }}">
        @csrf
        @if($isEdit)
            @method('PUT')
        @endif
        <div class="card">
            ...
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-3"><label class="form-label">วันที่ติดตาม <span class="text-danger">*</span></label><input type="date" name="followup_date" class="form-control" min="{{ $minimumFollowupDate ?? optional($plan->start_date)->format('Y-m-d') }}" max="{{ now('Asia/Bangkok')->format('Y-m-d') }}" value="{{ old('followup_date', optional($followup?->followup_date)->format('Y-m-d') ?? now('Asia/Bangkok')->format('Y-m-d')) }}" {{ $readOnly?'disabled':'' }} required></div>
                    <div class="col-md-3">
                        <label class="form-label">ประเภทการติดตาม</label>
                        <select name="followup_type" class="form-select" {{ $readOnly ? 'disabled' : '' }}>
                    ...
                    <div class="col-md-3">
                        <label class="form-label">ผลโดยรวม <span class="text-danger">*</span></label>
                        <select name="overall_result" class="form-select" {{ $readOnly ? 'disabled' : '' }} required>
                            @foreach($resultLabels as $key => $label)
                                <option value="{{ $key }}" @selected($selectedOverall === $key)>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <div class="card domain-card" data-domain="{{ $domain->id }}">
                ...
                <div class="card-body">
                    @foreach($domain->indicators as $indicator)
                        ...
                        @endphp
                        <div class="indicator" data-indicator="{{ $id }}" data-prev="{{ $prev ?? '' }}" data-rubrics='@json($rubricMap)'>
                            ...
                            <div class="detail-grid">
                                <div><label class="form-label">หลักฐาน/พฤติกรรมที่พบ <span class="evidence-star text-danger" style="display:none">*</span></label><textarea name="items[{{ $id }}][evidence]" class="form-control evidence-input" {{ $readOnly?'disabled':'' }}>{{ old('items.'.$id.'.evidence',$oldItem?->evidence) }}</textarea><div class="changed-note">คะแนนเปลี่ยนจากครั้งก่อน กรุณาระบุหลักฐานประกอบ</div></div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="section-grid">
                    ...
                    <div><label class="form-label">สิ่งที่ต้องดำเนินการต่อ <span class="text-danger" id="nextActionStar">*</span></label><textarea name="next_action" id="nextAction" class="form-control" {{ $readOnly?'disabled':'' }}>{{ old('next_action',$followup?->next_action) }}</textarea></div>
                </div>
                <div class="row g-3 mt-1"><div class="col-md-4"><label class="form-label">วันที่ติดตามครั้งถัดไป</label><input type="date" name="next_followup_date" class="form-control" value="{{ old('next_followup_date',optional($followup?->next_followup_date)->format('Y-m-d')) }}" {{ $readOnly?'disabled':'' }}></div><div class="small text-muted mt-1">หากผลโดยรวมเป็น “บรรลุเป้าหมาย” และไม่มีแผนติดตามต่อ สามารถเว้นวันที่ติดตามครั้งถัดไปและสิ่งที่ต้องดำเนินการต่อได้</div></div>
            </div>
        </div>
        ...
        <div class="sticky-actions">
            ...
            @unless($readOnly)
                <button type="submit" class="btnx primary">
                    <i class="bi bi-check2-circle"></i>{{ $isEdit ? 'บันทึกการแก้ไข' : 'บันทึกการติดตามครั้งที่ ' . $followupNo }}
                </button>
            @endunless
        </div>
    </form>
...
document.addEventListener('DOMContentLoaded',function(){
  function updateIndicator(box){
    var checked=box.querySelector('input[type=radio]:checked'); if(!checked)return;
    var level=checked.value,rubrics={}; try{rubrics=JSON.parse(box.dataset.rubrics||'{}')}catch(e){}
    var data=rubrics[level]||{},l=box.querySelector('.rubric-level'),t=box.querySelector('.rubric-text'); if(l)l.textContent=level;if(t)t.textContent=data.description||'-';
    var prev=Number(box.dataset.prev||0), changed=prev>0 && Number(level)!==prev, evidence=box.querySelector('.evidence-input'), star=box.querySelector('.evidence-star');
    box.classList.toggle('score-changed',changed); if(evidence)evidence.required=changed; if(star)star.style.display=changed?'inline':'none';
  }
  ...
  var overall=document.querySelector('select[name="overall_result"]'),next=document.getElementById('nextAction'),star=document.getElementById('nextActionStar');
  function syncNext(){var terminal=overall && overall.value==='achieved';if(next)next.required=!terminal;if(star)star.style.display=terminal?'none':'inline';}
  if(overall){overall.addEventListener('change',syncNext);syncNext();}
});
</script>
