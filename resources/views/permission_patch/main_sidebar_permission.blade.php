@php
    ...
    /*
    |--------------------------------------------------------------------------
    | สิทธิ์ทางเข้าทะเบียนผู้รับบริการ
    |--------------------------------------------------------------------------
    |
    | ผู้ใช้ที่เปิดสิทธิ์รายฟอร์มจะเห็นทะเบียนผู้รับบริการเมื่อได้รับสิทธิ์
    | ดูอย่างน้อยหนึ่งฟอร์ม เพื่อให้สามารถเลือกผู้รับบริการก่อนเข้าสู่ฟอร์มนั้นได้
    |
    */
    $allClientPermissionKeys = collect(config('user_permissions.groups', []))
        ->flatMap(fn (array $group) => array_keys($group['items'] ?? []))
        ->values()
        ->all();

    $canAccessClientRegistry = $permissionUser
        ? $permissionUser->hasAnyFormPermission($allClientPermissionKeys, 'view')
        : false;

    $canStatelessCentral = $permissionUser
        ? $permissionUser->hasFormPermission('welfare_stateless_person', 'view')
        : false;

    $canPrintStatelessCentral = $permissionUser
        ? $permissionUser->hasFormPermission('welfare_stateless_person', 'print')
        : false;

    $canViewDischargeReport = $permissionUser
        ? $permissionUser->hasFormPermission('welfare_discharge', 'view')
        : false;
